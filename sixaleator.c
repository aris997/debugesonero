 ...
 //PRINTING STANDARD
  ...
  for(i=0; i<MAX; i++){
    ...
    for(ii=0; ii<sum; ii++){
      ...
      r = lrand48()%sum;
      cella = finder(rubrica[r], L);
    
      fprintf(stderr, "name:%ld id:%d ", rubrica[r], r);
      ...
      if(r == 0){ //Nord-Est
        if(y%2 == 0 && alveare[pE[x]][N[y]] == 0) alveare[pE[x]][N[y]] = alveare[x][y];
        else if(alveare[dE[x]][N[y]] == 0) alveare[dE[x]][N[y]] = alveare[x][y];
        else error(4);
      }
      else if(r == 1){ //Est
        if(y%2 == 0 && alveare[pE[x]][y] == 0) alveare[pE[x]][y] = alveare[x][y];
        else if(alveare[dE[x]][y] == 0)alveare[dE[x]][y] = alveare[x][y];
        else error(4);
      }
      else if(r == 2){ //Sud-Est
        if(y%2 == 0 && alveare[pE[x]][S[y]] == 0) alveare[pE[x]][S[y]] = alveare[x][y];
